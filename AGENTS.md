...
| `.github/workflows/deploy.yml` | GitHub Pages 静态部署 + 资源 cache-bust | 直接部署 `book/` 下的 HTML，无需构建 |
| `.tmp/` | 临时与参照资料（git-ignored） | 不要提交 |

## 书籍结构与元数据

全书七部分、17 章 + 序言，**章节清单只维护一处：`book/common.js` 顶部的 `BOOK.chapters`**。它驱动页头面包屑、目录、章内 TOC、前后章导航、页脚。
...
- 新增/重排/改名章节，必须同步更新 `BOOK.chapters`（`file`、`no`、`title`、`part`），否则导航与目录会错乱。
- 部分名（如「第二部分 · Agent 核心」）同时出现在 `BOOK.chapters` 与章节页面的 `chapter-kicker` 中，两处保持一致。
...
- **不要**手写页头、目录、前后章导航、页脚——`common.js` 会根据 `BOOK.chapters` 自动生成，章节内只留 `<div id="pb-nav-holder"></div>`。
- 标题格式：`<title>` 用 `第 N 章 · 短标题 | Pi Agent 源码解析`；`<h1>` 用 `第 N 章：长标题`；`.chapter-sub` 是凝练的一句话副题。
...
1. 新章节出现在页头面包屑、目录、前后章导航中（即 `BOOK.chapters` 注册正确）。
...
- 改书籍元数据（标题、顺序、部分划分）只动 `common.js` 的 `BOOK.chapters`。
- 新增章节：复制现有章节骨架 → 写正文 → 注册进 `BOOK.chapters` → 浏览器验证（或跑 `.tmp/verify/` 的两个脚本）。
